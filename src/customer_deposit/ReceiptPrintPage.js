...
// ReceiptPrintPage – rendered by headless Chrome via the Go backend's
// /v1/receipt/pdf endpoint. It fetches the receipt data stored under
// a one-time key, renders the receipt with CustomerDepositPreviewContent,
...
function ReceiptPrintPage() {
    ...
    useEffect(() => {
        ...
        fetch(`/v1/receipt/print-data/${key}`)
            .then(res => {
    ...
    // After the receipt is rendered, wait for all fonts to finish loading,
