...
function CustomerDepositIndex(props) {
    ...
    useEffect(() => {
        const saved = localStorage.getItem("customer_deposit_table_settings");
        if (saved) {
        ...
        if (missingOrUpdated) {
            localStorage.setItem("customer_deposit_table_settings", JSON.stringify(defaultColumns));
    ...
    const handleToggleColumn = (index) => {
        ...
        setColumns(updated);
        localStorage.setItem("customer_deposit_table_settings", JSON.stringify(updated));
    };
    ...
    const onDragEnd = (result) => {
        ...
        localStorage.setItem("customer_deposit_table_settings", JSON.stringify(reordered));
    };
                                                    ...
                                                    <th key={col.key}>
                                                        ...
                                                        {col.key === "date" && (
                                                            <>
                                                                <DatePicker
                                                                    ...
                                                                    onKeyDown={(e) => {
                                                                        if (timerRef.current) clearTimeout(timerRef.current);
                                                                        if (e.key === "Escape") { timerRef.current = setTimeout(() => { setDateValue(""); searchByDateField("date_str", ""); }, 100); }
                                                                    }}
                                                                />
                                                        ...
                                                        {col.key === "net_total" && (
                                                            <input
                                                                ...
                                                                className="form-control"
                                                                id="receivable_net_total"
                                                                name="receivable_net_total"
                                                                ref={netTotalSearchRef}
                                                                onKeyDown={(e) => {
                                                                    if (timerRef.current) clearTimeout(timerRef.current);
                                                                    if (e.key === "Escape") { timerRef.current = setTimeout(() => { netTotalSearchRef.current.value = ""; searchByFieldValue("net_total", ""); }, 100); }
                                                                }}
                                                            />
                                                        )}
                                                        ...
                                                        {col.key === "description" && (
                                                            <input
                                                                ...
                                                                name="receivable_description"
                                                                ref={descriptionSearchRef}
                                                                onKeyDown={(e) => {
                                                                    if (timerRef.current) clearTimeout(timerRef.current);
                                                                    if (e.key === "Escape") { timerRef.current = setTimeout(() => { descriptionSearchRef.current.value = ""; searchByFieldValue("description", ""); }, 100); }
                                                                }}
                                                            />
                                                        )}
                                                        {col.key === "created_by" && (
                                                            ...
                                                            <React.Fragment key={col.key}>
                                                                {col.key === "actions" && (
                                                                    <td style={{ width: "auto", whiteSpace: "nowrap" }}>
                                                                        ...
                                                                        <Button className="btn btn-primary btn-sm" onClick={() => openDetailsView(customerdeposit.id)}><i className="bi bi-eye"></i></Button>&nbsp;
                                                                        <Button className="btn btn-primary btn-sm" onClick={() => openPreview(customerdeposit)}><i className="bi bi-printer"></i></Button>&nbsp;
                                                                        <Button className="btn btn-success btn-sm" onClick={() => sendWhatsAppMessage(customerdeposit)}>
                                                                ...
                                                                {col.fieldName === "code" && <td style={{ width: "auto", whiteSpace: "nowrap" }}>{customerdeposit.code}</td>}
                                                                {col.key === "date" && <td style={{ width: "auto", whiteSpace: "nowrap" }}>{format(new Date(customerdeposit.date), "MMM dd yyyy h:mma")}</td>}
                                                                {col.key === "type" && <td style={{ width: "auto", whiteSpace: "nowrap" }}>{customerdeposit.type}</td>}
                                                                {col.fieldName === "customer_name" && <td style={{ width: "auto", whiteSpace: "nowrap" }}>{customerdeposit.customer_name && <OverflowTooltip value={customerdeposit.customer_name + (customerdeposit.customer_name_arabic ? "|" + customerdeposit.customer_name_arabic : "")} maxWidth={300} />}</td>}
                                                                {col.fieldName === "vendor_name" && <td style={{ width: "auto", whiteSpace: "nowrap" }}>{customerdeposit.vendor_name && <OverflowTooltip value={customerdeposit.vendor_name + (customerdeposit.vendor_name_arabic ? "|" + customerdeposit.vendor_name_arabic : "")} maxWidth={300} />}</td>}
                                                                {col.fieldName === "net_total" && <td style={{ width: "auto", whiteSpace: "nowrap" }}><Amount amount={trimTo2Decimals(customerdeposit.net_total)} /></td>}
                                                                {col.key === "payment_methods" && <td style={{ width: "auto", whiteSpace: "nowrap" }}>{customerdeposit.payment_methods && customerdeposit.payment_methods.map((name, i) => <React.Fragment key={i}><span className="badge bg-info">{name}</span>&nbsp;</React.Fragment>)}</td>}
