                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-lg">
                        ...
                        <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            ...
                            <div style={{ marginTop: '4px', fontSize: '12px', color: '#ba1a1a', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                <i className="bi bi-arrow-up-circle" style={{ fontSize: '13px' }}></i>
                                Drawing / Withdrawal
                            </div>
                        </div>
                        ...
                        <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px' }}>
                                ...
                                <span style={{ fontSize: '18px', fontWeight: 600, color: '#191c1e', fontFamily: "'Hanken Grotesk', sans-serif" }}>
                                    {formatPaymentMethod(model.payment_method)}
                                </span>
                            </div>
                        </div>
                        ...
                        {/* Withdrawn By */}
                        <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            <span style={{ fontSize: '13px', fontWeight: 600, color: '#434655', lineHeight: '16px' }}>Withdrawn By</span>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px' }}>
                                <i className="bi bi-person-circle" style={{ fontSize: '20px', color: '#505f76' }}></i>
                                <span style={{ fontSize: '16px', fontWeight: 600, color: '#191c1e', fontFamily: "'Hanken Grotesk', sans-serif" }}>
                        ...
                        <div className="lg:col-span-8" style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                            ...
                            <section style={{ backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                                <div style={{ padding: '12px 24px', borderBottom: '1px solid #c3c6d7', display: 'flex', alignItems: 'center', gap: '8px', backgroundColor: '#f2f4f6' }}>
                                    ...
                                    <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 600, lineHeight: '26px', fontFamily: "'Hanken Grotesk', sans-serif", color: '#191c1e' }}>Drawing Details</h3>
                                </div>
                                <div style={{ padding: '24px', display: 'flex', flexDirection: 'column' }}>
                                    {[
                                        { label: 'Code', value: model.code, mono: true },
                                        { label: 'Amount', value: model.amount != null ? Number(model.amount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : null, bold: true },
                                        { label: 'Payment Method', value: formatPaymentMethod(model.payment_method) },
                                        { label: 'Withdrawn By', value: model.withdrawn_by_user_name },
                                        { label: 'Date', value: formatInStoreTimezone(model.date) },
                                        { label: 'Description', value: model.description },
                                    ].filter(r => r.value && r.value !== '—').map((r, i, arr) => (
                                        <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '10px 0', borderBottom: i < arr.length - 1 ? '1px solid #c3c6d7' : 'none', gap: '16px' }}>
                                            <span style={{ fontSize: '14px', color: '#434655', flexShrink: 0, minWidth: '140px' }}>{r.label}</span>
                                            <span style={{ fontSize: '14px', fontWeight: r.bold ? 700 : 500, color: r.mono ? '#004ac6' : '#191c1e', fontFamily: r.mono ? 'monospace' : 'inherit', textAlign: 'right' }}>{r.value}</span>
                                        </div>
                                    ))}
                                </div>
                            </section>
