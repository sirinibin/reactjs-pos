                ...
                <div className="p-md md:p-xl" style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-lg">
                        ...
                        {(() => {
                            ...
                            const totalExVAT = products.reduce((sum, p) => {
                                let profit = 0;
                            ...
                            const totalWithVAT = products.reduce((sum, p) => {
                                let profit = 0;
                                ...
                                <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    ...
                                    {npWithVAT > 0 && model.net_total > 0 && (
                                        <div style={{ fontSize: '12px', color: '#15803d', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                            ...
                                            {trimTo2Decimals((npWithVAT / model.net_total) * 100)}% {t("Margin")}
                                        </div>
                                    )}
                                </div>
                            );
                    ...
                    <section style={{ backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '32px' }}>
                        ...
                        {/* Totals Summary */}
                        {(() => {
                            const totalProductProfit = (model.products || []).reduce((sum, p) => {
                                let profit = 0;
                            ...
                            const deductions = (model.commission || 0) + (model.cash_discount || 0) + (model.discount || 0);
                            const netProfit = trimTo2Decimals(totalProductProfit - deductions);
                            const totalProductProfitWithVAT = (model.products || []).reduce((sum, p) => {
                                let profit = 0;
                            ...
                            const netProfitWithVAT = trimTo2Decimals(totalProductProfitWithVAT - deductions);
                            return (
                        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '24px', backgroundColor: '#ffffff' }}>
                            <div style={{ width: '100%', maxWidth: '320px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px', lineHeight: '20px', fontWeight: 600, paddingTop: '6px', borderTop: '1px solid #c3c6d7' }}>
                                    <span style={{ color: '#434655' }}>{t("Net Profit (ex-VAT)")}</span>
                                    <span style={{ color: netProfit >= 0 ? '#15803d' : '#ba1a1a' }}>
                                        <NumberFormat value={netProfit} displayType={"text"} thousandSeparator={true} renderText={(v) => v} />
                                    </span>
                                </div>
