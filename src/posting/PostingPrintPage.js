...
// PostingPrintPage – rendered by headless Chrome via the Go backend's
// /v1/posting/pdf endpoint. It fetches the posting data stored under
// a one-time key, renders the balance sheet with BalanceSheetPrintPreviewContent,
...
function PostingPrintPage() {
    ...
    useEffect(() => {
        ...
        fetch(`/v1/posting/print-data/${key}`)
            .then(res => {
    ...
    return (
        <>
            ...
            <div style={{ background: 'white' }}>
                <BalanceSheetPrintPreviewContent
                    model={model}
                    invoiceBackground=""
                    whatsAppShare={false}
                    modelName={modelName}
...
export default PostingPrintPage;
