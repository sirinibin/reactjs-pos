// Smoke test: SourceDocumentPicker renders without crashing
...
beforeEach(() => {
    ...
    // mockResolvedValue implementation assigned above before every test, so
    // it must be re-established here.
    global.fetch = jest.fn().mockResolvedValue({
        ok: true,
        headers: { get: () => 'application/json' },
        json: () => Promise.resolve({ result: [], data: [], total_count: 0, store: {}, settings: {} }),
...
describe('SourceDocumentPicker smoke test', () => {
    it('renders without crashing (modal hidden by default)', () => {
        const ref = createRef();
        render(
            <MemoryRouter>
                <SourceDocumentPicker ref={ref} />
            </MemoryRouter>
        );
    ...
    it('renders without crashing when modal is opened via ref', () => {
        ...
        if (ref.current && ref.current.open) {
            ref.current.open(jest.fn(), 'quotation', []);
        }
        // No crash is the assertion
    });

    it('renders without crashing when opened for purchase doc type', () => {
        const ref = createRef();
        render(
            <MemoryRouter>
                <SourceDocumentPicker ref={ref} />
        ...
        if (ref.current && ref.current.open) {
            ref.current.open(jest.fn(), 'purchase', []);
        }
        // No crash is the assertion
    });
});
