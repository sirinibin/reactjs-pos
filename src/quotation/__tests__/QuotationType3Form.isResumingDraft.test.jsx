...
describe('QuotationType3Form — isResumingDraft fix (was: wasResumingDraft)', () => {
    test('renders without crashing — no ReferenceError on isResumingDraft', () => {
        expect(() =>
            render(<MemoryRouter><QuotationType3Form /></MemoryRouter>)
        ).not.toThrow();
    });

    test('renders with openDetailsView prop (update-mode path)', () => {
        expect(() =>
            render(<MemoryRouter><QuotationType3Form openDetailsView={jest.fn()} /></MemoryRouter>)
        ).not.toThrow();
    });

    test('renders with showToastMessage prop', () => {
        expect(() =>
            render(<MemoryRouter><QuotationType3Form showToastMessage={jest.fn()} /></MemoryRouter>)
        ).not.toThrow();
    });

    test('no console error referencing wasResumingDraft on mount', async () => {
        let caught = null;
        const orig = console.error;
        console.error = (...args) => {
            const msg = typeof args[0] === 'string' ? args[0] : '';
            if (msg.includes('wasResumingDraft')) caught = msg;
            orig(...args);
        };
        await act(async () => {
            render(<MemoryRouter><QuotationType3Form openDetailsView={jest.fn()} /></MemoryRouter>);
    ...
    test('onDraftSaved prop is called correctly when provided', () => {
        const onDraftSaved = jest.fn();
        expect(() =>
            render(<MemoryRouter><QuotationType3Form onDraftSaved={onDraftSaved} /></MemoryRouter>)
        ).not.toThrow();
    });

    test('onDraftCreated prop is called correctly when provided', () => {
        const onDraftCreated = jest.fn();
        expect(() =>
            render(<MemoryRouter><QuotationType3Form onDraftCreated={onDraftCreated} /></MemoryRouter>)
        ).not.toThrow();
    });
});
