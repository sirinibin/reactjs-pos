...
const QuotationCreate = forwardRef((props, ref) => {
    ...
    fetch("/v1/quotation/" + id + "?" + queryParams, requestOptions)
      .then(async (response) => {
        ...
        if (quotation.customer_id) {
          const fallback = { ...(quotation.customer || {}), id: quotation.customer_id };
  ...
  async function suggestCustomers(searchTerm) {
    ...
    let Select =
      "select=id,code,credit_balance,credit_limit,additional_keywords,vat_no,name,phone,phone2,email,name_in_arabic,phone_in_arabic,search_label";
...
async function checkWarning(i) {
  ...
  */
  ...
  function RestoreDefaultSettings() {
    ...
    localStorage.setItem("quotation_product_search_settings", JSON.stringify(clonedDefaults));
  ...
  useEffect(() => {
    ...
    const savedRaw = localStorage.getItem("quotation_product_search_settings");
    let savedParsed = null;
    try {
      savedParsed = savedRaw ? JSON.parse(savedRaw) : null;
    } catch (_) {
      savedParsed = null;
    }

    if (savedParsed && Array.isArray(savedParsed)) {
      setSearchProductsColumns(savedParsed);
    } else {
      // No valid saved settings, hydrate with defaults and persist once
      setSearchProductsColumns(clonedDefaults);
      localStorage.setItem("quotation_product_search_settings", JSON.stringify(clonedDefaults));
    }

    // Handle structure changes: if columns changed (added/renamed), reset to defaults once
    if (savedParsed && Array.isArray(savedParsed)) {
      let missingOrUpdated = false;
      for (let i = 0; i < clonedDefaults.length; i++) {
        const savedCol = savedParsed.find(col => col.fieldName === clonedDefaults[i].fieldName);
        if (!savedCol || savedCol.label !== clonedDefaults[i].label || savedCol.key !== clonedDefaults[i].key) {
          missingOrUpdated = true;
          break;
        }
      }
  ...
  let [showQuotationSPSettings, setShowQuotationSPSettings] = useState(false);
  const defaultQuotationSPColumns = [
  ...
  const [quotationSPColumns, setQuotationSPColumns] = useState(() => {
    try {
      const saved = localStorage.getItem('quotation_sp_table_settings');
      if (saved) {
        const parsed = JSON.parse(saved);
        const merged = defaultQuotationSPColumns.map(def => {
    ...
    return defaultQuotationSPColumns;
  });
  useEffect(() => {
    localStorage.setItem('quotation_sp_table_settings', JSON.stringify(quotationSPColumns));
  }, [quotationSPColumns]);
  const handleToggleQuotationSPColumn = (key) => {
    setQuotationSPColumns(cols => cols.map(c => c.key === key ? { ...c, visible: !c.visible } : c));
  };
  const onDragEndQuotationSP = (result) => {
    if (!result.destination) return;
    const items = Array.from(quotationSPColumns);
    const [reordered] = items.splice(result.source.index, 1);
    items.splice(result.destination.index, 0, reordered);
    setQuotationSPColumns(items);
  };
  const restoreDefaultQuotationSPSettings = () => {
    setQuotationSPColumns(defaultQuotationSPColumns);
    localStorage.removeItem('quotation_sp_table_settings');
  };

  const [formType, setFormType] = useState(() => localStorage.getItem('quotation_form_type') || 'type1');
  useEffect(() => { localStorage.setItem('quotation_form_type', formType); }, [formType]);
  useEffect(() => {
    if (store?.settings?.quotation_create_form_design) {
      setFormType(store.settings.quotation_create_form_design);
    }
          ...
          <form className="row g-3 needs-validation" onSubmit={handleCreate}>
            {!enableProductSelection && <>
              ...
              <div className="col-12">
                <div className="entity-header-grid" style={{ gap: '10px', alignItems: 'start' }}>
                  ...
                  {/* LEFT: customer Typeahead + form fields */}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '231px 1fr 1fr', gap: '8px 65px', alignItems: 'start', maxWidth: '80%', marginTop: '10px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <input
                            id="quotation_phone" name="quotation_phone"
                      ...
                      <div>
                        ...
                        <input
                          id="quotation_vat_no" name="quotation_vat_no"
                      ...
                      {/* R2C1: Barcode Scan */}
                      <div>
                        <label className="form-label" style={{ fontSize: '12px', marginBottom: '2px' }}>Product Barcode Scan</label>
                        <DebounceInput
                          minLength={3}
                          debounceTimeout={100}
                        ...
                        />
                        {errors.bar_code && <div style={{ color: "red" }}>{errors.bar_code}</div>}
                      </div>
            ...
            <section style={{ backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '8px' }}>
              <div className="sc-header-flex" style={{ borderBottom: '1px solid #c3c6d7' }}>
                {/* Left: customer search + date + phone + VAT + remarks */}
              ...
              <table className="sc-type2-table" style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px', tableLayout: 'fixed' }}>
                <colgroup>
                  {quotationSPColumns.filter(c => c.visible).map(col => (
                    <col key={col.key} style={{ width: `${scColWidths[col.key] ?? SC_COL_DEFAULTS_Q[col.key] ?? 100}px` }} />
                  ))}
                </colgroup>
                <thead style={{ backgroundColor: '#f1f5f9', position: 'sticky', top: 0, zIndex: 1 }}>
                  {(() => {
                    ...
                    const resizeHandle = (colKey) => (
                      <div
                        onMouseDown={(e) => startScColResize(e, colKey, scColWidths[colKey] ?? SC_COL_DEFAULTS_Q[colKey] ?? 60)}
                    ...
                    return (
                      <tr style={{ fontSize: '12px', fontWeight: 600, color: '#434655', lineHeight: '16px' }}>
                        {quotationSPColumns.filter(c => c.visible).map(col => {
                          ...
                          if (col.key === 'info') return <th key="info" style={thStyle}>Info{resizeHandle('info')}</th>;
                          if (col.key === 'purchase_unit_price') return <th key="purchase_unit_price" style={{ ...thStyle, textAlign: 'right' }}>P. Unit Price{resizeHandle('purchase_unit_price')}</th>;
                          if (col.key === 'stock') return <th key="stock" style={thStyle}>Stock{resizeHandle('stock')}</th>;
                          if (col.key === 'warehouse') return store.settings?.enable_warehouse_module ? <th key="warehouse" style={thStyle}>Warehouse{resizeHandle('warehouse')}</th> : null;
                          if (col.key === 'qty') return <th key="qty" style={{ ...thStyle, textAlign: 'center' }}>Qty{resizeHandle('qty')}</th>;
                    ...
                    return (
                      <tr key={index}
                        ...
                        {quotationSPColumns.filter(c => c.visible).map(col => {
                                ...
                                <input type="number"
                                  ...
                                  onKeyDown={(e) => {
                                    ...
                                    if (e.key === "Enter") {
                                      ...
                                      e.preventDefault();
                                      const refMap = { purchase_unit_price: 'quotation_product_purchase_unit_price_', qty: 'quotation_product_quantity_', unit_price: 'quotation_product_unit_price_', unit_price_with_vat: 'quotation_product_unit_price_with_vat_', unit_discount: 'quotation_unit_discount_' };
                                      const ci = quotationSPColumns.findIndex(c => c.key === 'unit_discount_with_vat');
                                      let target = null;
                                      for (let i = ci - 1; i >= 0 && !target; i--) { const col = quotationSPColumns[i]; if (col.visible && refMap[col.key]) target = inputRefs.current[index]?.[refMap[col.key] + index] || null; }
                          ...
                          if (col.key === 'unit_discount_percent_with_vat') return (<td key="unit_discount_percent_with_vat" style={{ verticalAlign: 'middle', padding: '0.25rem' }}>
                            <div className="d-flex align-items-center" style={{ minWidth: 0 }}>
                              <div className="input-group flex-nowrap" style={{ flex: '1 1 auto', minWidth: 0 }}>
                                <input type="number"
                ...
                <thead>
                  <tr className="text-center">
                    {quotationSPColumns.filter(c => c.visible).map(col => {
                      ...
                      if (col.key === 'warehouse') return (store.settings?.enable_warehouse_module && formData.type === "invoice") ? <th key="warehouse">Remove Stock From</th> : null;
                      if (col.key === 'qty') return <th key="qty">Qty</th>;
              ...
              <div className="row" style={{ marginTop: "12px" }}>
                <div className="col-md-2">
                  ...
                  <input
                    type='number'
                    ref={commissionRef}
                    id="quotation_commission"
                    name="quotation_commission"
                    value={commission}
                    className="form-control"
                    onChange={(e) => {
      ...
      <Modal show={showQuotationSPSettings} onHide={() => setShowQuotationSPSettings(false)} size="md">
        ...
        <Modal.Body>
          <DragDropContext onDragEnd={onDragEndQuotationSP}>
            <Droppable droppableId="quotation-sp-columns">
              {(provided) => (
                <ul className="list-group" {...provided.droppableProps} ref={provided.innerRef}>
                  {quotationSPColumns.map((col, idx) => (
