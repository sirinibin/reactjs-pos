...
const QuotationSalesReturnCreate = forwardRef((props, ref) => {
    ...
    */
    ...
    const defaultQSRSPColumns = [
        { key: 'select', label: 'Select / Select All', visible: true },
    ...
    const [qsrSPColumns, setQsrSPColumns] = useState(() => {
        try {
            const saved = localStorage.getItem('qsr_sp_table_settings');
            if (saved) {
                const parsed = JSON.parse(saved);
                const merged = defaultQSRSPColumns.map(def => {
        ...
        return defaultQSRSPColumns;
    });
    useEffect(() => {
        localStorage.setItem('qsr_sp_table_settings', JSON.stringify(qsrSPColumns));
    }, [qsrSPColumns]);
    const handleToggleQSRSPColumn = (key) => {
        setQsrSPColumns(cols => cols.map(c => c.key === key ? { ...c, visible: !c.visible } : c));
    };
    const onDragEndQSRSP = (result) => {
        if (!result.destination) return;
        const items = Array.from(qsrSPColumns);
        const [reordered] = items.splice(result.source.index, 1);
        items.splice(result.destination.index, 0, reordered);
        setQsrSPColumns(items);
    };
    const restoreDefaultQSRSPSettings = () => {
        setQsrSPColumns(defaultQSRSPColumns);
        localStorage.removeItem('qsr_sp_table_settings');
    };

    const [formType, setFormType] = useState(() => localStorage.getItem('quotation_sales_return_form_type') || 'type1');
    useEffect(() => { localStorage.setItem('quotation_sales_return_form_type', formType); }, [formType]);
    useEffect(() => {
        if (store?.settings?.quotation_sales_return_create_form_design) {
            setFormType(store.settings.quotation_sales_return_create_form_design);
        }
    }, [store?.settings?.quotation_sales_return_create_form_design]);
    const SC_COL_DEFAULTS_QSR = { select: 50, si_no: 40, part_number: 100, name: 200, info: 50, purchase_unit_price: 130, stock: 60, qty: 70, warehouse: 130, unit_price: 130, unit_price_with_vat: 130, unit_discount: 120, unit_discount_with_vat: 120, unit_discount_percent_with_vat: 90, price: 120, price_with_vat: 120 };
    const [scColWidths, setScColWidths] = useState(() => { try { return JSON.parse(localStorage.getItem('qsr_sc_col_widths')) || {}; } catch { return {}; } });
    useEffect(() => { localStorage.setItem('qsr_sc_col_widths', JSON.stringify(scColWidths)); }, [scColWidths]);
                    ...
                    {selectedProducts?.length > 0 && <form className="needs-validation" onSubmit={handleCreate}>
                        ...
                        <section style={{ backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '8px' }}>
                            <div className="sc-header-flex" style={{ borderBottom: '1px solid #c3c6d7' }}>
                                <div className="sc-header-left" style={{ padding: '4px 10px', display: 'flex', gap: '6px', alignItems: 'stretch', backgroundColor: '#f2f4f6', borderRight: '1px solid #c3c6d7' }}>
                                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        ...
                                        <div className="sc-sub-row sc-customer-row" style={{ alignItems: 'center', flexWrap: 'nowrap' }}>
                                            <div className="sc-customer-search-group" style={{ display: 'flex', gap: '4px', flex: 1, minWidth: 0 }}>
                                                ...
                                                <button type="button" onClick={() => CustomerCreateFormRef.current.open()}
                                                ...
                                                {formData.customer_id && (
                                                    <button type="button" onClick={() => CustomerCreateFormRef.current.open(formData.customer_id)}
                                                        style={{ background: '#fff', border: '1px solid #c3c6d7', borderRadius: '4px', padding: '7px 12px', fontSize: '13px', cursor: 'pointer', flexShrink: 0 }}>
                                                        <i className="bi bi-pencil" />
                                                    </button>
                                                )}
                                ...
                                {selectedCustomers && selectedCustomers.length > 0 && formData.customer_id ? (() => {
                                    const c = selectedCustomers[0];
                                    const sep = <span style={{ width: '1px', height: '12px', background: '#c3c6d7', flexShrink: 0 }} />;
                                    const phone = c.phone || formData.phone;
                                    const vatNo = c.vat_no || formData.vat_no;
                                    return (
                                        <div className="sc-header-right" style={{ padding: '4px 14px', background: 'rgba(0,74,198,0.03)', borderLeft: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '6px', overflow: 'hidden', minHeight: '40px' }}>
                                            {c.code && <span style={{ background: '#dbeafe', color: '#1e40af', borderRadius: '4px', padding: '1px 7px', fontSize: '12px', fontWeight: 700, flexShrink: 0 }}>{c.code}</span>}
                                            <span style={{ fontWeight: 700, fontSize: '15px', color: '#191c1e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: 0, maxWidth: '280px' }}>{c.name}</span>
                                            {c.name_in_arabic && <><span style={{ color: '#c3c6d7', fontSize: '13px', flexShrink: 0 }}>|</span><span style={{ fontSize: '13px', color: '#64748b' }}>{c.name_in_arabic}</span></>}
                                            {(phone || vatNo) && sep}
                                            {phone && <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '13px', color: '#374151', flexShrink: 0 }}><i className="bi bi-telephone" style={{ color: '#6b7280', fontSize: '12px' }} />{phone}</span>}
                                            {phone && vatNo && sep}
                                            {vatNo && <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '13px', color: '#374151', flexShrink: 0 }}><i className="bi bi-receipt" style={{ color: '#6b7280', fontSize: '12px' }} /><span style={{ color: '#6b7280' }}>VAT:</span>{vatNo}</span>}
                        ...
                        <div style={{ overflowX: 'auto', maxHeight: '55vh', overflowY: 'auto', marginTop: '6px' }}>
                            <table className="sc-type2-table" style={{ width: '100%', borderCollapse: 'collapse', minWidth: '600px', tableLayout: 'fixed' }}>
                                <colgroup>{qsrSPColumns.filter(c => c.visible).map(col => <col key={col.key} style={{ width: `${scColWidths[col.key] ?? SC_COL_DEFAULTS_QSR[col.key] ?? 100}px` }} />)}</colgroup>
                                        ...
                                        return (
                                            <tr style={{ fontSize: '12px', fontWeight: 600, color: '#434655', lineHeight: '16px' }}>
                                                {qsrSPColumns.filter(c => c.visible).map(col => {
                                                    ...
                                                    if (col.key === 'info') return <th key={col.key} style={thStyle}>Info{resizeHandle('info')}</th>;
                                                    if (col.key === 'purchase_unit_price') return <th key={col.key} style={{ ...thStyle, textAlign: 'right' }}>P. Unit Price{resizeHandle('purchase_unit_price')}</th>;
                                                    if (col.key === 'stock') return <th key={col.key} style={thStyle}>Stock{resizeHandle('stock')}</th>;
                                                    if (col.key === 'warehouse') return store.settings?.enable_warehouse_module ? <th key={col.key} style={thStyle}>Add Stock To{resizeHandle('warehouse')}</th> : null;
                                                    if (col.key === 'qty') return <th key={col.key} style={{ ...thStyle, textAlign: 'center' }}>Qty{resizeHandle('qty')}</th>;
                                    ...
                                    {selectedProducts.map((product, index) => {
                                        ...
                                        return (
                                            <tr key={index} style={{ borderBottom: '1px solid #e2e8f0', transition: 'background-color 0.15s' }}
                                                ...
                                                {qsrSPColumns.filter(c => c.visible).map(col => {
                                                    if (col.key === 'select') return (<td key="select" style={{ verticalAlign: 'middle', padding: '0.25rem', width: "20px", whiteSpace: "nowrap" }}>
                                                        <input
                                                    ...
                                                    if (col.key === 'part_number') return (<td key="part_number" style={{ verticalAlign: 'middle', padding: '0.25rem', width: "auto", whiteSpace: "nowrap" }}>
                                ...
                                <tbody>
                                    <tr className="text-center" style={{ borderBottom: "solid 2px" }}>
                                        {qsrSPColumns.filter(c => c.visible).map(col => {
                                            ...
                                            if (col.key === 'warehouse') return store.settings?.enable_warehouse_module ? <th key="warehouse">Add Stock To</th> : null;
                                                            ...
                                                            <div className="input-group flex-nowrap" style={{ flex: '1 1 auto', minWidth: 0 }}>
                                                                <input type="number"
                                                                    ...
                                                                    onChange={(e) => {
                                                                        ...
                                                                        selectedProducts[index].unit_discount_percent_with_vat = parseFloat(e.target.value);
            ...
            <Modal show={showQSRSPSettings} onHide={() => setShowQSRSPSettings(false)} size="md">
                ...
                <Modal.Body>
                    <DragDropContext onDragEnd={onDragEndQSRSP}>
                        <Droppable droppableId="qsr-sp-columns">
                            {(provided) => (
                                <ul className="list-group" {...provided.droppableProps} ref={provided.innerRef}>
                                    {qsrSPColumns.map((col, idx) => (
                ...
                <Modal.Footer>
                    <Button variant="secondary" onClick={restoreDefaultQSRSPSettings}>Restore Defaults</Button>
                    <Button variant="primary" onClick={() => setShowQSRSPSettings(false)}>Close</Button>
                </Modal.Footer>
            </Modal>
