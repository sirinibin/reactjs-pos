...
const StockTransferIndex = forwardRef((props, ref) => {
    ...
    let [sortStockTransfer, setSortStockTransfer] = useState("-");


    const [totalStockTransferAmount, setTotalStockTransferAmount] = useState(0.00);
    const [totalStockTransferQuantity, setTotalStockTransferQuantity] = useState(0.00);
    ...
    const list = useCallback(() => {
        ...
        let Select =
            "select=id,code,date,net_total,total_quantity,created_by_name,from_warehouse_code,to_warehouse_code,created_at,store_id";
        ...
        fetch(
            ...
            .then(async (response) => {
                ...
                setStockTransferList(data.result);
                ...
                setTotalStockTransferAmount(data.meta.total_stocktransfer);
                setTotalStockTransferQuantity(data.meta.total_quantity);
            })
            .catch((error) => {
                setIsListLoading(false);
                setIsRefreshInProcess(false);
    ...
    }, [sortStockTransfer, sortField, page, pageSize, statsOpen, searchParams]);
    ...
    useEffect(() => {
        if (lastMessage) {
            ...
            if (jsonMessage.event === "stocktransfer_updated") {
    ...
    useEffect(() => {
        const handleSocketOpen = () => {
            //console.log("WebSocket Opened in stocktransfer list");
            list();
        };
    ...
    function sort(field) {
        ...
        sortStockTransfer = sortStockTransfer === "-" ? "" : "-";
        setSortStockTransfer(sortStockTransfer);
        // alert(field + "," + sortStockTransfer);
        list();
    }
    ...
    */
    ...
    let [showStockTransferView, setShowStockTransferView] = useState(false);

    const DetailsViewRef = useRef();
    function openDetailsView(id) {
        showStockTransferView = true;
        setShowStockTransferView(true);
        if (timerRef.current) clearTimeout(timerRef.current);
    ...
    let [showStockTransferCreateForm, setShowStockTransferCreateForm] = useState(false);
    const CreateFormRef = useRef();
    function openCreateForm() {
        showStockTransferCreateForm = true;
        setShowStockTransferCreateForm(true);
    ...
    const openPrintTypeSelection = useCallback((stocktransfer) => {
    ...
    /*function openReportPreview() {
         ...
         if (timerRef.current) clearTimeout(timerRef.current);
         timerRef.current = setTimeout(() => {
             ReportPreviewRef.current?.open("stocktransfer_report");
         }, 100);
     }*/

    function sendWhatsAppMessage(model) {
        showStockTransferPreview = true;
        setShowStockTransferPreview(showStockTransferPreview);
    ...
    return (
        <>
            ...
            <Modal show={showPrintTypeSelection} onHide={() => {
                ...
                <Modal.Body className="d-flex justify-content-around">
                    <Button variant="secondary" ref={printButtonRef} onClick={() => {
                        openPrint(selectedStockTransfer);
                    }} onKeyDown={(e) => {
                        if (timerRef.current) clearTimeout(timerRef.current);
                    ...
                    <Button variant="primary" ref={printA4ButtonRef} onClick={() => {
                        openPreview(selectedStockTransfer);
                    }}
                        onKeyDown={(e) => {
                            if (timerRef.current) clearTimeout(timerRef.current);
            ...
            {/*showReportPreview && <ReportPreview ref={ReportPreviewRef} searchParams={searchParams} sortStockTransfer={sortStockTransfer} sortField={sortField} />*/}
                                                ...
                                                {columns.filter(c => c.visible).map((col) => {
                                                    return (<>
                                                        ...
                                                        {col.key !== "actions" &&
                                                            ...
                                                            <th><input
                                                                type="text"
                                                                id={"stocktransfer_" + col.fieldName}
                                                                name={"stocktransfer_" + col.fieldName}
                                                                onChange={(e) =>
                                                            ...
                                                            return (<>
                                                                {(col.key === "actions" || col.key === "actions_end") && <td style={{ width: "auto", whiteSpace: "nowrap" }}>
                                                                    ...
                                                                    <Button className="btn btn-primary btn-sm" onClick={() => {
                                                                        openDetailsView(stocktransfer.id);
                                                                    }}>
                                                                        <i className="bi bi-eye"></i>
                                                                    </Button>&nbsp;
                                                                    <Button className="btn btn-primary btn-sm" onClick={() => {
                                                                        openPrintTypeSelection(stocktransfer);
                                                                    }}>
                                                                        <i className="bi bi-printer"></i>
                                                                    </Button>
                                                                    &nbsp;
                                                                    <Button className={`btn btn-success btn-sm`} style={{}} onClick={() => {
                                                                        sendWhatsAppMessage(stocktransfer);
                                                                    }}>
                                                                ...
                                                                {(col.fieldName === "net_total") && <td style={{ width: "auto", whiteSpace: "nowrap" }}>
                                                                    <Amount amount={trimTo2Decimals(stocktransfer.net_total)} />
                                                                </td>}
