...
import "./print.css";
import WhatsAppModal from '../utils/WhatsAppModal.js';
...
const Preview = forwardRef((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        async open(modelObj, whatsapp, modelNameStr) {
            ...
            if (modelObj) {
                ...
                if (model.stocktransfer_id) {
                    await getStockTransfer(model.stocktransfer_id);
                }
    ...
    /*
     {props.model.store?.settings?.hide_quotation_invoice_vat ? <>
                                                    {(props.modelName === "quotation" && props.model.type === "invoice") || (props.modelName === "whatsapp_quotation" && props.model.type === "invoice") || props.modelName === "quotation_stocktransfers_return" || props.modelName === "whatsapp_quotation_stocktransfers_return" ? "" : <Amount amount={trimTo2Decimals(props.model.vat_price)} />}
    ...
    */
    ...
    function setHideVAT(modelName) {
        if (model.store?.settings.hide_quotation_invoice_vat) {
            if (((modelName === "quotation" || modelName === "whatsapp_quotation") && model.type === "invoice") || modelName === "quotation_stocktransfers_return" || modelName === "whatsapp_quotation_stocktransfers_return") {
                model.hideVAT = true;
            } else {
                model.hideVAT = false;
            }
    ...
    function setInvoiceTitle(modelName) {
        ...
        if (model.modelName === "stocktransfers" || model.modelName === "whatsapp_stocktransfers" || model.modelName === "stocktransfers_return" || model.modelName === "whatsapp_stocktransfers_return") {
        ...
        if (model.modelName === "stocktransfers" || model.modelName === "whatsapp_stocktransfers") {
            if (model.store?.zatca?.phase === "1") {
                if (model.payment_status !== "not_paid") {
                    //model.invoiceTitle = "TAX INVOICE | الفاتورة الضريبية";
                    model.invoiceTitle = model.store.settings.invoice.phase1.stocktransfers_titles.paid;
                    if (IsCashOnly) {
                        // model.invoiceTitle = "CASH TAX INVOICE | فاتورة ضريبية نقدية";
                        model.invoiceTitle = model.store.settings.invoice.phase1.stocktransfers_titles.cash;
                    }
                } else if (model.payment_status === "not_paid") {
                    // model.invoiceTitle = "CREDIT TAX INVOICE | فاتورة ضريبة الائتمان";
                    model.invoiceTitle = model.store.settings.invoice.phase1.stocktransfers_titles.credit;
                }
            } else if (model.store?.zatca?.phase === "2") {
                if (isSimplified) {
                    if (model.payment_status === "not_paid") {
                        // model.invoiceTitle = "SIMPLIFIED CREDIT TAX INVOICE | فاتورة ضريبة الائتمان المبسطة";
                        model.invoiceTitle = model.store.settings.invoice.phase2.stocktransfers_titles.credit;
                    } else {
                        //  model.invoiceTitle = "SIMPLIFIED TAX INVOICE | فاتورة ضريبية مبسطة";
                        model.invoiceTitle = model.store.settings.invoice.phase2.stocktransfers_titles.paid;
                        if (IsCashOnly) {
                            // model.invoiceTitle = "SIMPLIFIED CASH TAX INVOICE | فاتورة ضريبية نقدية مبسطة";
                            model.invoiceTitle = model.store.settings.invoice.phase2.stocktransfers_titles.cash;
                        }
                    }
                } else {
                    if (model.payment_status === "not_paid") {
                        // model.invoiceTitle = "STANDARD CREDIT TAX INVOICE | فاتورة ضريبة الائتمان القياسية";
                        model.invoiceTitle = model.store.settings?.invoice?.phase2_b2b?.stocktransfers_titles?.credit;
                    } else {
                        //model.invoiceTitle = "STANDARD TAX INVOICE | فاتورة ضريبية قياسية";
                        model.invoiceTitle = model.store.settings?.invoice?.phase2_b2b?.stocktransfers_titles?.paid;
                        if (IsCashOnly) {
                            // model.invoiceTitle = "STANDARD CASH TAX INVOICE | فاتورة ضريبية نقدية قياسية";
                            model.invoiceTitle = model.store.settings?.invoice?.phase2_b2b?.stocktransfers_titles?.cash;
                        }
                    }
                }
            }
        } else if (model.modelName === "stocktransfers_return" || model.modelName === "whatsapp_stocktransfers_return") {
            if (model.store?.zatca?.phase === "1") {
                //model.invoiceTitle = "STOCKTRANSFERS RETURN TAX INVOICE | فاتورة ضريبة المبيعات المرتجعة";
                model.invoiceTitle = model.store.settings?.invoice?.phase1?.stocktransfers_return_titles?.paid;
                if (IsCashOnly) {
                    //model.invoiceTitle = "STOCKTRANSFERS RETURN CASH TAX INVOICE | إقرار مبيعات فاتورة ضريبية نقدية";
                    model.invoiceTitle = model.store.settings?.invoice?.phase1?.stocktransfers_return_titles?.cash;
                }
            } else if (model.store?.zatca?.phase === "2") {
                if (isSimplified) {
                    // model.invoiceTitle = "SIMPLIFIED CREDIT NOTE RETURN TAX INVOICE | إقرار ضريبي مبسط لإقرار إقرار ائتماني";
                    model.invoiceTitle = model.store.settings?.invoice?.phase2?.stocktransfers_return_titles?.paid;
                    if (IsCashOnly) {
                        // model.invoiceTitle = "SIMPLIFIED CREDIT NOTE CASH RETURN TAX INVOICE | مذكرة ائتمان مبسطة، إقرار نقدي، فاتورة ضريبية";
                        model.invoiceTitle = model.store.settings?.invoice?.phase2?.stocktransfers_return_titles?.cash;
                    }

                    if (model.payment_status === "not_paid") {
                        model.invoiceTitle = model.store.settings?.invoice?.phase2?.stocktransfers_return_titles?.credit;
                    }
                } else {
                    // model.invoiceTitle = "STANDARD CREDIT NOTE RETURN TAX INVOICE | إقرار ضريبي قياسي لإرجاع فاتورة الائتمان";
                    model.invoiceTitle = model.store.settings?.invoice?.phase2_b2b?.stocktransfers_return_titles?.paid;
                    if (IsCashOnly) {
                        // model.invoiceTitle = "STANDARD CREDIT NOTE CASH RETURN TAX INVOICE | سند ائتمان قياسي، إقرار نقدي، فاتورة ضريبية";
                        model.invoiceTitle = model.store.settings?.invoice?.phase2_b2b?.stocktransfers_return_titles?.cash;
                    }

                    if (model.payment_status === "not_paid") {
                        model.invoiceTitle = model.store.settings?.invoice?.phase2_b2b?.stocktransfers_return_titles?.credit;
                    }
                }
            }
            ...
            if (model.type === "invoice" && model.payment_status === "not_paid") {
                ...
                model.invoiceTitle = model.store.settings?.invoice?.quotation_stocktransfers_titles.credit;
                /*if (model.store.code === "LGK-SIMULATION" || model.store.code === "LGK") {
                   // model.invoiceTitle = "CREDIT STOCKTRANSFERS STOCKTRANSFER | أمر مبيعات الائتمان";

                }*/
            } else if (model.type === "invoice") {
                //model.invoiceTitle = "INVOICE | فاتورة";
                model.invoiceTitle = model.store.settings?.invoice?.quotation_stocktransfers_titles.paid;
                /*if (model.store.code === "LGK-SIMULATION" || model.store.code === "LGK") {
                    model.invoiceTitle = "STOCKTRANSFERS STOCKTRANSFER | أمر المبيعات";
                }*/
            }

        } else if (model.modelName === "quotation_stocktransfers_return" || model.modelName === "whatsapp_quotation_stocktransfers_return") {
            if (model.payment_status === "not_paid") {
                // model.invoiceTitle = "CREDIT RETURN INVOICE | فاتورة إرجاع الائتمان";
                model.invoiceTitle = model.store.settings?.invoice?.quotation_stocktransfers_return_titles.credit;
            } else {
                // model.invoiceTitle = "RETURN INVOICE | فاتورة الإرجاع";
                model.invoiceTitle = model.store.settings?.invoice?.quotation_stocktransfers_return_titles.paid;
            }
    ...
    let [modelName, setModelName] = useState("stocktransfers");
    ...
    function preparePages() {
        ...
        if (modelName === "stocktransfers_return" || modelName === "whatsapp_stocktransfers_return" || modelName === "purchase_return" || modelName === "whatsapp_purchase_return" || modelName === "quotation_stocktransfers_return" || modelName === "whatsapp_quotation_stocktransfers_return") {
        ...
        for (let i = 0; i < totalPages; i++) {
            ...
            for (let j = offset; j < totalProducts; j++) {
                /* if (modelName == "stocktransfers_return" && !model.products[j].selected) {
                     continue
                 }*/
    ...
    async function getModel(id, modelName) {
        ...
        let apiPath = "";
        if (modelName && (modelName === "stocktransfers" || modelName === "whatsapp_stocktransfers")) {
            apiPath = "stocktransfer"
        } else if (modelName && (modelName === "stocktransfers_return" || modelName === "whatsapp_stocktransfers_return")) {
            apiPath = "stocktransfers-return"
            ...
            apiPath = "quotation"
        } else if (modelName && (modelName === "quotation_stocktransfers_return" || modelName === "whatsapp_quotation_stocktransfers_return")) {
            apiPath = "quotation-stocktransfers-return"
    ...
    async function getStockTransfer(id) {
        ...
        await fetch('/v1/stocktransfer/' + id + "?" + queryParams, requestOptions)
            .then(async response => {
                const isJson = response.headers.get('content-type')?.includes('application/json');
                const data = isJson && await response.json();
                ...
                model.stocktransfer = data.result;
                setModel({ ...model });
    ...
    */
    ...
    const getFileName = useCallback(() => {
        ...
        if (modelName === "stocktransfers" || modelName === "whatsapp_stocktransfers") {
            filename += "StockTransfers";
        } else if (modelName === "stocktransfers_return" || modelName === "whatsapp_stocktransfers_return") {
            filename += "StockTransfers_Return";
            ...
            filename += "Quotation";
        } else if (modelName === "quotation_stocktransfers_return" || modelName === "whatsapp_quotation_stocktransfers_return") {
            filename += "Qtn_StockTransfers_Return";
    ...
    const handleDownload = async () => {
        ...
        formData.append("id", model.id); // stocktransfer id
        formData.append("store_id", model.store_id); // stocktransfer id
    ...
    useEffect(() => {
        ...
        let modelNames = [
            "stocktransfers",
            "whatsapp_stocktransfers",
            "stocktransfers_return",
            "whatsapp_stocktransfers_return",
            "purchase",
            "whatsapp_purchase",
            "purchase_return",
            ...
            "quotation",
            "whatsapp_quotation",
            "quotation_stocktransfers_return",
            "whatsapp_quotation_stocktransfers_return",
            "delivery_note",
        ...
        <Modal show={show} scrollable={true} size="xl" fullscreen onHide={handleClose} animation={false}>
            <Modal.Header className="d-flex flex-wrap align-items-center justify-content-between">
                ...
                <div className="d-flex flex-wrap align-items-center" style={{ gap: '10px' }}>
                    {/* Slider */}
                    {showSlider && (
                        <div className="d-flex align-items-center bstocktransfer rounded bg-light p-2">
                    ...
                    <div className="d-flex align-items-center bstocktransfer rounded bg-light p-2" style={{ marginRight: "200px" }}>
