...
import StockTransferPrintContent from './printContent.js';
import StockTransferPrintContent2 from './printContent2.js';
import StockTransferPrintContent3 from './printContent3.js';
import StockTransferPrintContent4 from './printContent4.js';

import { useReactToPrint } from 'react-to-print';
import { Invoice } from '@axenda/zatca';
//import { format } from "date-fns";

const StockTransferPrint = forwardRef((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        async open(modelObj, modelNameStr) {
            ...
            if (modelObj) {
                ...
                /*
                if (modelObj.id) {
                    await getStockTransfer(modelObj.id);
                }*/
    ...
    function getQRCodeContents() {
        ...
        if (model.code) {
            qrContent += "StockTransfers Invoice #: " + model.code + "<br />";
        }
    ...
    */
    ...
    function getFileName() {
        let filename = "StockTransfers_Invoice";
    ...
    return (<>
        <Modal show={show} scrollable={true} size="xl" fullscreen={model.store?.code === "PH2" || model.store?.code === "LGK-SIMULATION" || model.store?.code === "LGK"} onHide={handleClose} animation={false} style={{ overflowY: "auto", height: "auto" }}>
            <Modal.Header className="d-flex flex-wrap align-items-center justify-content-between">
                ...
                <div className="row" style={{ bstocktransfer: "solid 0px" }}>
                    <div className="col align-self-end text-end" style={{ bstocktransfer: "solid 0px", minWidth: "200px" }}>
                        <>
                    ...
                    <div className="col align-self-end text-end" style={{ bstocktransfer: "solid 0px", minWidth: "100px" }}>
            ...
            <Modal.Body>
                <div ref={printAreaRef} >
                    {(model.store?.code === "GUOJ-T" || model.store?.code === "UMLJ-T") && <StockTransferPrintContent
                        model={model}
                    />}
                    {(model.store?.code === "PH2" || model.store?.code === "LGK-SIMULATION" || model.store?.code === "LGK") && <StockTransferPrintContent2
                        model={model}

                    />}
                    {(model.store?.code === "YNB-SIMULATION" || model.store?.code === "YNB") && <StockTransferPrintContent3
                        model={model}
                    />}
                    {(model.store?.code === "GUOJ" || model.store?.code === "UMLJ" || model.store?.code === "JDA-SIMULATION" || model.store?.code === "JDA") && <StockTransferPrintContent4
                        model={model}
                    />}
                </div>
            </Modal.Body>
