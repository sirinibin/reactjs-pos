...
const ReportPreview = forwardRef((props, ref) => {
    let [modelName, setModelName] = useState("stocktransfers_report");
    ...
    /*
    const [totalStockTransfers, setTotalStockTransfers] = useState(0.00);
    const [netProfit, setNetProfit] = useState(0.00);
    const [vatPrice, setVatPrice] = useState(0.00);
    const [totalPaidStockTransfers, setTotalPaidStockTransfers] = useState(0.00);
    const [totalUnPaidStockTransfers, setTotalUnPaidStockTransfers] = useState(0.00);
    const [totalCashStockTransfers, setTotalCashStockTransfers] = useState(0.00);
    const [totalBankAccountStockTransfers, setTotalBankAccountStockTransfers] = useState(0.00);
    ...
    */
    ...
    async function getAllModels() {
        ...
        } else if (modelName === "quotation_stocktransfers_return_report") {
            Select =
                "select=id,code,date,net_total,total_payment_received,balance_amount,customer_name,payment_status";
        ...
        let apiNameSpace = "";
        if (modelName === "stocktransfers_report") {
            apiNameSpace = "stocktransfer"
        } else if (modelName === "stocktransfers_return_report") {
            apiNameSpace = "stocktransfers-return"
        } else if (modelName === "purchase_report") {
            ...
            apiNameSpace = "quotation-stocktransfers-return"
        } else if (modelName === "delivery_note_report") {
            ...
            let res = await fetch(
                ...
                .then(async (response) => {
                    ...
                    if (data.meta) {
                        ...
                        /*
                        setTotalStockTransfers(data.meta.total_stocktransfers);
                        setNetProfit(data.meta.net_profit);
                        setLoss(data.meta.net_loss);
                        setVatPrice(data.meta.vat_price);
                        setTotalPaidStockTransfers(data.meta.paid_stocktransfers);
                        setTotalUnPaidStockTransfers(data.meta.unpaid_stocktransfers);
                        setTotalCashStockTransfers(data.meta.cash_stocktransfers);
                        setTotalBankAccountStockTransfers(data.meta.bank_account_stocktransfers);
    ...
    function setInvoiceTitle(modelName) {
        ...
        if (model.modelName === "stocktransfers_report") {
            model.reportTitle = "STOCKTRANSFERS REPORT | تقرير المبيعات"
        } else if (model.modelName === "stocktransfers_return_report") {
            model.reportTitle = "STOCKTRANSFERS RETURN REPORT | تقرير مرتجعات المبيعات"
        ...
        } else if (model.modelName === "quotation_stocktransfers_return_report") {
            model.reportTitle = "QTN. STOCKTRANSFERS RETURN REPORT | تقرير مرتجعات المبيعات"
    ...
    /*
        async function getModel(id, modelName) {
            ...
            let apiPath = "";
            if (modelName && modelName === "stocktransfers") {
                apiPath = "stocktransfer"
            } else if (modelName && modelName === "stocktransfers_return") {
    ...
    const getFileName = useCallback(() => {
        ...
        if (modelName === "stocktransfers_report") {
            filename = "StockTransfers_Report";
        } else if (modelName === "stocktransfers_return_report") {
            filename = "StockTransfers_Return_Report";
        } else if (modelName === "purchase_report") {
            ...
            filename = "Qtn_StockTransfers_Return_Report";
        } else if (modelName === "delivery_note_report") {
    ...
    */
    ...
    useEffect(() => {
        ...
        let modelNames = ["stocktransfers", "stocktransfers_report", "stocktransfers_return", "stocktransfers_return_report", "quotation_stocktransfers_return_report", "purchase", "purchase_report", "purchase_return", "purchase_return_report", "quotation", "quotation_report", "quotation_invoice_report", "delivery_note", "delivery_note_report"];
